...
namespace NightVision
{
    ...
    public class StatWorker_NightVision : StatWorker
    {
        ...
        {
            if (req.HasThing && req.Thing is Pawn pawn && pawn.GetComp<Comp_NightVision>() is Comp_NightVision comp)
            {
                ...
                if (usedApparelSetting)
                {
                    ...
                    {
                        if(NightVisionSettings.NVApparel.TryGetValue(app.def, out ApparelSetting setting) && setting.GrantsNV)
                        {
    ...
    public class StatWorker_LightSensitivity : StatWorker
    {
        public override string GetExplanationUnfinalized(StatRequest req, ToStringNumberSense numberSense)
        {
            if (req.HasThing && req.Thing is Pawn pawn && pawn.GetComp<Comp_NightVision>() is Comp_NightVision comp)
            {
                ...
                if (usedApparelSetting)
                {
                    ...
                    foreach (Apparel app in comp.PawnsNVApparel ?? Enumerable.Empty<Apparel>())
                    {
                        if (NightVisionSettings.NVApparel.TryGetValue(app.def, out ApparelSetting setting) && setting.NullifiesPS)
                        {
                            builder.AppendLine(app.LabelCap);
                        }
                    }
        ...
        public override string GetStatDrawEntryLabel(StatDef stat, float value, ToStringNumberSense numberSense, StatRequest optionalReq)
        {
            if (optionalReq.HasThing && optionalReq.Thing is Pawn pawn && pawn.GetComp<Comp_NightVision>() is Comp_NightVision comp)
            {
                string result =
