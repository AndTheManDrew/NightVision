        ...
        internal class NightVisionSettings : ModSettings
            {
                ...
                private const  float LowestCap                  = 0.01f;
                private const  float HighestCap                 = 2f;
                ...
                [UsedImplicitly]
                public NightVisionSettings() => Instance = this;
                ...
                private static List<ThingDef> GetAllHeadgear
                    {
                        get
                            {
                ...
                private static List<HediffDef> GetAllHediffs
                    {
                        get
                            {
                ...
                public static Dictionary<ThingDef, ApparelSetting> NVApparel =
                            new Dictionary<ThingDef, ApparelSetting>();
                ...
                public void DoSettingsWindowContents(
                    Rect inRect)
                    {
                        TabsList.Clear();
                        ...
                        if (Prefs.DevMode)
                            {
                                TabsList.Add(new TabRecord("Debug",
                    ...
                    Rect inRect)
                    {
                        ...
                        Widgets.Label(rowRect,
                            "NVMoveWorkSpeedMultipliers"
                                        .Translate(LightModifiersBase.Options.NVNightVision.ToString().Translate())
                                        .ToLower()
                                        .CapitalizeFirst());
                        rowRect.y += rowHeight + RowGap;
                        _nvZeroCache = Widgets.HorizontalSlider(rowRect,
                            (float) _nvZeroCache,
                            (float) _minCache,
                            (float) _maxCache,
                            true,
                            string.Format(ZeroMultiLabel, _nvZeroCache),
                            string.Format(XLabel,         _minCache),
                            string.Format(XLabel,         _maxCache),
                            1);
                        DrawIndicator(rowRect,
                            DefaultZeroLightMultiplier,
                            LightModifiersBase.NVDefaultOffsets[0],
                            (float) _minCache,
                            (float) _maxCache,
                            IndicatorTex.DefIndicator);
                        rowRect.y += rowHeight * 1.5f;
                        _nvFullCache = Widgets.HorizontalSlider(rowRect,
                            (float) _nvFullCache,
                            (float) _minCache,
                            (float) _maxCache,
                            true,
                            string.Format(FullMultiLabel, _nvFullCache),
                            string.Format(XLabel,         _minCache),
                            string.Format(XLabel,         _maxCache),
                            1);
                        DrawIndicator(rowRect,
                            DefaultFullLightMultiplier,
                            LightModifiersBase.NVDefaultOffsets[1],
                            (float) _minCache,
                            (float) _maxCache,
                            IndicatorTex.DefIndicator);
                        ...
                        Widgets.Label(rowRect,
                            "NVMoveWorkSpeedMultipliers"
                                        .Translate(LightModifiersBase.Options.NVPhotosensitivity.ToString().Translate())
                                        .ToLower()
                                        .CapitalizeFirst());
                        rowRect.y += rowHeight * 1.5f;
                        _psZeroCache = Widgets.HorizontalSlider(rowRect,
                            (float) _psZeroCache,
                            (float) _minCache,
                            (float) _maxCache,
                            true,
                            string.Format(ZeroMultiLabel, _psZeroCache),
                            string.Format(XLabel,         _minCache),
                            string.Format(XLabel,         _maxCache),
                            1);
                        DrawIndicator(rowRect,
                            DefaultZeroLightMultiplier,
                            LightModifiersBase.PSDefaultOffsets[0],
                            (float) _minCache,
                            (float) _maxCache,
                            IndicatorTex.DefIndicator);

                        rowRect.y += rowHeight * 1.5f;
                        _psFullCache = Widgets.HorizontalSlider(rowRect,
                            (float) _psFullCache,
                            (float) _minCache,
                            (float) _maxCache,
                            true,
                            string.Format(FullMultiLabel, _psFullCache),
                            string.Format(XLabel,         _minCache),
                            string.Format(XLabel,         _maxCache),
                            1);
                        DrawIndicator(rowRect,
                            DefaultFullLightMultiplier,
                            LightModifiersBase.PSDefaultOffsets[1],
                            (float) _minCache,
                            (float) _maxCache,
                            IndicatorTex.DefIndicator);
                        ...
                        if (_customCapsEnabled)
                            {
                                ...
                                Text.Font =  GameFont.Small;
                                rowRect.y += rowHeight + RowGap;
                                _minCache = Widgets.HorizontalSlider(rowRect,
                                    (float) _minCache,
                                    1f,
                                    100f,
                                    true,
                                    "NVSettingsMinCapLabel".Translate(_minCache),
                                    "1%",
                                    "100%",
                                    1);
                                DrawIndicator(rowRect, DefaultMinCap, 0f, 1f, 100f, IndicatorTex.DefIndicator);
                                rowRect.y += rowHeight * 1.5f;
                                _maxCache = Widgets.HorizontalSlider(rowRect,
                                    (float) _maxCache,
                                    100f,
                                    200f,
                                    true,
                                    "NVSettingsMinCapLabel".Translate(_maxCache),
                                    "100%",
                                    "200%",
                                    1);
                        ...
                        if (CEDetected)
                            {
                                Widgets.CheckboxLabeled(rowRect,
                                    ...
                                    ref HarmonyPatches.NVEnabledForCE);
                            }
                ...
                private void DrawRaceTab(
                    Rect inRect)
                    {
                        ...
                        if (_numberOfCustomRaces == null)
                            {
                                _numberOfCustomRaces =
                                            RaceLightMods.Count(
                                                rlm => rlm.Value.IntSetting == LightModifiersBase.Options.NVCustom);
                            }
                        ...
                        var viewRect = new Rect(inRect.x,
                            inRect.y,
                            inRect.width * 0.9f,
                            raceCount * (RowHeight + RowGap) + (float) _numberOfCustomRaces * 100f);
                        var rowRect = new Rect(inRect.x + 6f, num, inRect.width - 12f, RowHeight);
                        Widgets.BeginScrollView(inRect, ref _raceScrollPosition, viewRect);
                        var count = 0;
                        foreach (KeyValuePair<ThingDef, Race_LightModifiers> kvp in RaceLightMods)
                            {
                                ...
                                _numberOfCustomRaces +=
                                            DrawLightModifiersRow(kvp.Key, kvp.Value, rowRect, ref num, true);
                                if (!kvp.Value.ShouldShowInSettings)
                                    {
                        ...
                        foreach (ThingDef appareldef in GetAllHeadgear)
                            {
                                ...
                                if (NVApparel.TryGetValue(appareldef, out ApparelSetting apparelSetting))
                                    {
                ...
                private void DrawHediffTab(
                    Rect inRect)
                    {
                        ...
                        if (_numberOfCustomHediffs == null)
                            {
                                _numberOfCustomHediffs =
                                            HediffLightMods.Count(
                                                hlm => hlm.Value.IntSetting == LightModifiersBase.Options.NVCustom);
                            }
                        ...
                        var viewRect = new Rect(inRect.x,
                            inRect.y,
                            inRect.width * 0.9f,
                            hediffcount * (RowHeight + RowGap) + (float) _numberOfCustomHediffs * 100f);
                        var rowRect = new Rect(inRect.x + 6f, num, inRect.width - 12f, RowHeight);
                        ...
                        for (var i = 0; i < hediffcount; i++)
                            {
                                ...
                                if (HediffLightMods.TryGetValue(hediffdef, out Hediff_LightModifiers hediffmods))
                                    {
                                        _numberOfCustomHediffs += DrawLightModifiersRow(hediffdef,
                                            hediffmods,
                                            rowRect,
                                            ref num,
                                            false);
                                    ...
                                    {
                                        Hediff_LightModifiers temp = AllEyeHediffs.Contains(hediffdef)
                                                    ? new Hediff_LightModifiers {AffectsEye = true}
                                                    : new Hediff_LightModifiers();
                                        _numberOfCustomHediffs +=
                                                    DrawLightModifiersRow(hediffdef, temp, rowRect, ref num, false);
                ...
                private void DrawLightModifiersHeader(
                    ref Rect inRect,
                    string   label)
                    {
                        ...
                        var settingsRect = new Rect(xSettings,
                            headerRect.y,
                        ...
                        Widgets.Label(settingsRect,
                            new GUIContent("default".Translate().ToLower().CapitalizeFirst(),
                                IndicatorTex.DefIndicator));
                        settingsRect.x += num;
                        Widgets.Label(settingsRect,
                            new GUIContent("NVNightVision".Translate().ToLower().CapitalizeFirst(),
                                IndicatorTex.NvIndicator));
                        settingsRect.x += num;
                        Widgets.Label(settingsRect,
                            new GUIContent("NVPhotosensitivity".Translate().ToLower().CapitalizeFirst(),
                                IndicatorTex.PsIndicator));
                        Text.Font = font;
                        Widgets.DrawLineHorizontal(headerRect.x + 10f,
                            headerRect.yMax + RowGap / 4 - 0.5f,
                ...
                private int DrawLightModifiersRow(
                    Def                def,
                    ...
                    bool               isRace)
                    {
                        ...
                        TooltipHandler.TipRegion(labelRect,
                            new TipSignal(GetTipString(def, lightMods), labelRect.GetHashCode()));
                        Widgets.DrawLineVertical(x, rowRect.y, rowRect.height);
                        //LightModifiers.Options =  enum: default = 0; nightvis = 1; photosens = 2; custom = 3
                        for (var i = 0; i < 4; i++)
                            {
                                var iOption =
                                            (LightModifiersBase.Options) Enum.ToObject(
                                                typeof(LightModifiersBase.Options),
                                                i);
                                x += buttongap;
                        ...
                        if (lightMods.IsCustom())
                            {
                                num += RowHeight + RowGap;
                                var topRect = new Rect(labelRect.xMax + 2 * buttongap,
                                    num,
                                    rowRect.width - labelRect.width - 60f,
                                    RowHeight / 2);
                                var bottomRect = new Rect(labelRect.xMax + 2 * buttongap,
                                    topRect.yMax + RowGap * 2,
                                    rowRect.width - labelRect.width - 60f,
                                    RowHeight / 2);

                                var explanationRect = new Rect(labelRect.xMax - labelRect.width * 0.95f,
                                    num,
                                    labelRect.width * 0.9f,
                                    RowHeight);
                                Widgets.DrawLineVertical(labelRect.xMax + buttongap,
                                    rowRect.y + rowRect.height,
                                    RowHeight * 2 - RowGap * 0.5f);


                                var zeroModAsPercent =
                                            (float) Math.Round(
                                                (lightMods.Offsets[0] + (isRace ? DefaultZeroLightMultiplier : 0))
                                                * 100);
                                var fullModAsPercent =
                                            (float) Math.Round(
                                                (lightMods.Offsets[1] + (isRace ? DefaultFullLightMultiplier : 0))
                                                * 100);
                                ...
                                if (isRace)
                                    {
                                        ...
                                        if (((Race_LightModifiers) lightMods).CanCheat)
                                            {
                                                min = (float) Math.Round(LowestCap * 100);
                                                max = (float) Math.Round(HighestCap * 100);
                                            }
                                        else
                                            {
                                                min = (float) _minCache;
                                        ...
                                        zeroModAsPercent = Widgets.HorizontalSlider(topRect,
                                            zeroModAsPercent,
                                            min,
                                            max,
                                            true,
                                            string.Format(ZeroMultiLabel, zeroModAsPercent),
                                            string.Format(XLabel,         min),
                                            string.Format(XLabel,         max),
                                            1);

                                        fullModAsPercent = Widgets.HorizontalSlider(bottomRect,
                                            fullModAsPercent,
                                            min,
                                            max,
                                            true,
                                            string.Format(FullMultiLabel, fullModAsPercent),
                                            string.Format(XLabel,         min),
                                            string.Format(XLabel,         max),
                                            1);
                                ...
                                else
                                    {
                                        ...
                                        zeroModAsPercent = Widgets.HorizontalSlider(topRect,
                                            ...
                                            (float) _maxCache - 80,
                                            true,
                                            string.Format(ZeroLabel, zeroModAsPercent),
                                            string.Format(Alabel,    _minCache - 80),
                                            string.Format(Alabel,    _maxCache - 80),
                                            1);
                                        ...
                                        fullModAsPercent = Widgets.HorizontalSlider(bottomRect,
                                            ...
                                            (float) _maxCache - 100,
                                            true,
                                            string.Format(FullLabel, fullModAsPercent),
                                            string.Format(Alabel,    _minCache - 100),
                                            string.Format(Alabel,    _maxCache - 100),
                                            1);

                                        DrawIndicators(topRect,
                                            bottomRect,
                                            lightMods,
                                            (float) _minCache,
                                            (float) _maxCache,
                                ...
                                if (!Mathf.Approximately(zeroModAsPercent / 100, lightMods.Offsets[0]))
                                    {
                                        lightMods[0] =
                                                    zeroModAsPercent / 100 - (isRace ? DefaultZeroLightMultiplier : 0);
                                    }

                                if (!Mathf.Approximately(fullModAsPercent / 100, lightMods.Offsets[1]))
                                    {
                                        lightMods[1] =
                                                    fullModAsPercent / 100 - (isRace ? DefaultFullLightMultiplier : 0);
                                    }
                ...
                private void DrawDebugTab(
                    Rect inRect)
                    {
                        ...
                        listing.LabelDouble("NVDebugAverageTime".Translate(),
                            (HarmonyPatches.TotalGlFactorNanoSec / HarmonyPatches.TotalTicks).ToString("00 ns/tick"));
                        ...
                        if (playing)
                            {
                                if (_allPawns == null)
                                    {
                                        _allPawns = PawnsFinder
                                                    .AllMaps_Spawned.Where(pwn => pwn.RaceProps.Humanlike)
                                                    .ToList();
                                    }
                                ...
                                var viewRect = new Rect(remainRect.x + 6f,
                                    remainRect.y,
                                    remainRect.width - 12f,
                                    height);
                                        ...
                                        if (pawn.GetComp<Comp_NightVision>() is Comp_NightVision comp)
                                            {
                                                ...
                                                foreach (KeyValuePair<string, List<HediffDef>> hedifflist in comp
                                                            .PawnsNVHediffs)
                                                    {
                                                        rightRect.y += RowHeight / 2;
                                                        Widgets.Label(rightRect,
                                                        ...
                                                        foreach (HediffDef hediff in hedifflist.Value)
                                                            {
                                                                if (HediffLightMods.TryGetValue(hediff,
                                                ...
                                                foreach (Apparel apparel in comp.PawnsNVApparel)
                                                    {
                                                        if (NVApparel.TryGetValue(apparel.def,
                                                            out ApparelSetting appSet))
                                                            {
                                                                rightRect.y += RowHeight / 2;
                                                                Widgets.Label(rightRect,
                ...
                private static float ModToMultiPercent(
                    float mod,
                    bool  isZeroLight) =>
                            (float) Math.Round(
                                100f * (mod + (isZeroLight ? DefaultZeroLightMultiplier : DefaultFullLightMultiplier)));

                private static float MultiPercentToMod(
                    float multipercent,
                    bool  isZeroLight) =>
                ...
                private void ApparelDictionaryScribe(
                    ref Dictionary<ThingDef, ApparelSetting> dictionary)
                    {
                        if (dictionary == null)
                            {
                                return;
                        ...
                        if (Scribe.mode == LoadSaveMode.Saving)
                            {
                                foreach (KeyValuePair<ThingDef, ApparelSetting> kvp in dictionary)
                                    {
                ...
                private void LightModifiersDictionaryScribe<TK, TV>(
                ...
                internal void DoPreWriteTasks()
                    {
                        ...
                        if (_cacheInited)
                            {
                                MultiplierCaps.min = _minCache != null
                                            ? (float) Math.Round((float) _minCache / 100, 2)
                                            : MultiplierCaps.min;
                                MultiplierCaps.max = _maxCache != null
                                            ? (float) Math.Round((float) _maxCache / 100, 2)
                                            : MultiplierCaps.max;
