...
namespace NightVision
    {
        ...
        public static class NVHarmonyPatcher
            {
                ...
                static NVHarmonyPatcher()
                    ...
                    // Targets
                        ...
                        //ApparelTracker
                        MethodInfo ApparelTracker_Wear = AccessTools.Method(typeof(Pawn_ApparelTracker),
                            nameof(Pawn_ApparelTracker.Wear));
                        MethodInfo ApparelTracker_TryDrop = AccessTools.Method(typeof(Pawn_ApparelTracker),
                            nameof(Pawn_ApparelTracker.TryDrop),
                            new[] {typeof(Apparel), typeof(Apparel).MakeByRefType(), typeof(IntVec3), typeof(bool)});
                        MethodInfo ApparelTracker_Remove = AccessTools.Method(typeof(Pawn_ApparelTracker),
                    ...
                    //Patches
                        ...
                        //Hediff
                        harmony.Patch(Hediff_PostAdd,
                            null,
                            new HarmonyMethod(thistype, nameof(Hediff_PostAdd_Postfix)));
                        harmony.Patch(Hediff_PostRemoved,
                            null,
                            new HarmonyMethod(thistype, nameof(Hediff_PostRemoved_Postfix)));
                        //HediffWithComps
                        harmony.Patch(HediffWithComps_PostAdd,
                            null,
                            new HarmonyMethod(thistype, nameof(HediffWithComps_PostAdd_Postfix)));

                        //HediffDef
                        harmony.Patch(HediffDef_SpecialDisplayStats,
                            null,
                            new HarmonyMethod(thistype, nameof(SpecialDisplayStats_Postfix)));
            ...
            harmony.Patch(ApparelTracker_Wear, null, new HarmonyMethod(thistype, nameof(Wear_Postfix)));
                        harmony.Patch(ApparelTracker_TryDrop,
                            null,
                            new HarmonyMethod(thistype, nameof(TryDrop_Postfix)));
            ...
            //catch (TypeLoadException) { }
                ...
                public static void ExplanationPart_PostFix(
                    ...
                    ref string      __result)
                    {
                        if (!__result.NullOrEmpty() && req.Thing is Pawn pawn
                ...
                public static void HediffWithComps_PostAdd_Postfix(
                    HediffWithComps __instance)
                    {
                        if (__instance.pawn is Pawn pawn && pawn.Spawned /*&& pawn.RaceProps.Humanlike*/
                            ...
                            {
                                Log.Message("NightVision.NVHarmonyPatcher.HediffWithComps_PostAdd_Postfix: " + pawn + ", " + __instance.def);
                    ...
                    Hediff __instance)
                    {
                        if (__instance?.pawn is Pawn pawn && pawn.Spawned /*&& pawn.RaceProps.Humanlike*/
                                                          && pawn.TryGetComp<Comp_NightVision>() is Comp_NightVision
                                                                      comp)
                            {
                                Log.Message("NightVision.NVHarmonyPatcher.Hediff_PostAdd_Postfix: " + pawn + ", " + __instance.def);
                ...
                public static void Hediff_PostRemoved_Postfix(
                    Hediff __instance)
                    {
                        if (__instance?.pawn is Pawn pawn && pawn.Spawned /*&& pawn.RaceProps.Humanlike*/
                            ...
                            {
                                Log.Message("NightVision.NVHarmonyPatcher.Hediff_PostRemoved_Postfix: " + pawn + ", " + __instance.def);
                ...
                public static IEnumerable<StatDrawEntry> SpecialDisplayStats_Postfix(
                    ...
                    HediffDef              __instance)
                    {
                        ...
                        if (Storage.HediffLightMods.TryGetValue(__instance, out Hediff_LightModifiers hlm) && hlm.HasModifier())
                            {
                                VisionType vt = hlm.Setting;
                                if (vt < VisionType.NVCustom /*TODO review: && vt != VisionType.NVNone*/)
                                    {
                                        yield return new StatDrawEntry(StatCategoryDefOf.Basics, "NVGrantsVisionType".Translate(), vt.ToString().Translate(), 0, hlm.AffectsEye? "NVHediffQualifier".Translate() : "" );
                                    }
                                else
                                    {
                                        //TODO Condense? need to review the code for the stats
                                        yield return new StatDrawEntry(StatCategoryDefOf.Basics, "NVGrantsVisionType".Translate(), vt.ToString(), 0, hlm.AffectsEye? "NVHediffQualifier".Translate() : "" );
                                        yield return new StatDrawEntry(StatCategoryDefOf.Basics, NightVisionStatDefOf.NightVision, hlm[0], StatRequest.ForEmpty(), ToStringNumberSense.Offset);
                                        yield return new StatDrawEntry(StatCategoryDefOf.Basics, NightVisionStatDefOf.LightSensitivity, hlm[1], StatRequest.ForEmpty(), ToStringNumberSense.Offset);
                                    }
                            }
                    }
            ...
            Pawn_HealthTracker __instance)
        {
            ...
            if (pawn != null && pawn.Spawned
                             ...
                             pawn.TryGetComp<Comp_NightVision>() is Comp_NightVision comp)
            {
                Log.Message("NightVision.NVHarmonyPatcher.AddHediff_Postfix: " + pawn + ", " + hediff.def);
        ...
        public static void GetBodySize_Patch(
                ...
                public static void TryDrop_Postfix(
                    ...
                    ref bool            __result)
                    {
                        if (__result && __instance?.pawn is Pawn pawn && pawn.Spawned && pawn.RaceProps.Humanlike
                            && pawn.TryGetComp<Comp_NightVision>() is Comp_NightVision comp)
                            {
                ...
                public static void Remove_Postfix(
                    ...
                    Pawn_ApparelTracker __instance)
                    {
                        if (__instance?.pawn is Pawn pawn && pawn.RaceProps.Humanlike && pawn.Spawned
                            && pawn.TryGetComp<Comp_NightVision>() is Comp_NightVision comp)
                            {
                                comp.RemoveApparel(ap);
                            }
                ...
                public static void TakeWearoutDamageForTheDay_Postfix(
                    ...
                    Pawn_ApparelTracker __instance)
                    {
                        if (ap.Destroyed && __instance.pawn is Pawn pawn && pawn.RaceProps.Humanlike
                            && pawn.TryGetComp<Comp_NightVision>() is Comp_NightVision comp)
                            {
            ...
            List<CodeInstruction> inserts = new List<CodeInstruction>
                        {
                            ...
                            new CodeInstruction(OpCodes.Call,
                                AccessTools.Method(typeof(ThoughtWorker_TooBright),
